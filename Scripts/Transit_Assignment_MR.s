...
;  Transit Line Files           = MODE[1-10][AM|OP].TBe
...
;  Output Files: WKMRAMnode.dbf; WKMRAMlink.dbf
; Step 2: AM Peak Drive Assignment
;  Input Files: ZONEHWY.NET, MODE?_AM.TB, *.TB, '%_iter_%_AMMS.TRP'
;  Output Files: DRMRAMnode.dbf; DRMRAMlink.dbf
; Step 3: AM Peak K/R Assignment
;  Input Files: ZONEHWY.NET, MODE?_AM.TB, *.TB, '%_iter_%_AMMS.TRP'
;  Output Files: KRMRAMnode.dbf; KRMRAMlink.dbf
; Step 4: Off Peak Walk Assignment
;  Input Files: ZONEHWY.NET, MODE?_OP.TB, *.TB, '%_iter_%_OPMS.TRP'
;  Output Files: WKMROPnode.dbf; WKMROPlink.dbf
; Step 5: Off Peak Drive Assignment
;  Input Files: ZONEHWY.NET, MODE?_OP.TB, *.TB, '%_iter_%_OPMS.TRP'
;  Output Files: DRMROPnode.dbf; DRMROPlink.dbf
; Step 6: Off Peak K/R Assignment
;  Input Files: ZONEHWY.NET, MODE?_OP.TB, *.TB, '%_iter_%_OPMS.TRP'
;  Output Files: KRMROPnode.dbf; KRMROPlink.dbf
;---------------------------------------------------------------------------

;  Read in time factors to increase local bus times
;  based on increasing arterial hwy congestion 

READ FILE=INPUTS\LBus_TimFTRS.ASC   ; Local Bus Time Factors
...
;---------------------------------------------------------------------------
; Step 1, 2, 3, 4, 5 & 6 Assign MR Transit Trips
;---------------------------------------------------------------------------
...
fileo supporto = supl_MR_@access_mode@_@time_period@.asc modes=11-16 oneway=t fixed=y
;
...
;@DRIVE_MODEL@READ FILE = com__@TIME_PERIOD@.asc;--- drive to Commuter rail
...
; output files
@WALK_MODEL@@AM@FILEO NODEO  = %_iter_%_WKMRAMnode.dbf ; output node file
@WALK_MODEL@@OP@FILEO NODEO  = %_iter_%_WKMROPnode.dbf ; output node file
@DRIVE_MODEL@@AM@FILEO NODEO = %_iter_%_DRMRAMnode.dbf ; output node file
@DRIVE_MODEL@@OP@FILEO NODEO = %_iter_%_DRMROPnode.dbf ; output node file
@KR_MODEL@@AM@FILEO NODEO    = %_iter_%_KRMRAMnode.dbf ; output node file
@KR_MODEL@@OP@FILEO NODEO    = %_iter_%_KRMROPnode.dbf ; output node file

@WALK_MODEL@@AM@FILEO LINKO  = %_iter_%_WKMRAMlink.dbf ; output link file
@WALK_MODEL@@OP@FILEO LINKO  = %_iter_%_WKMROPlink.dbf ; output link file
@DRIVE_MODEL@@AM@FILEO LINKO = %_iter_%_DRMRAMlink.dbf ; output link file
@DRIVE_MODEL@@OP@FILEO LINKO = %_iter_%_DRMROPlink.dbf ; output link file
@KR_MODEL@@AM@FILEO LINKO    = %_iter_%_KRMRAMlink.dbf ; output link file
@KR_MODEL@@OP@FILEO LINKO    = %_iter_%_KRMROPlink.dbf ; output link file
