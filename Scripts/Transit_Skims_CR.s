...
;  Output Files: transit.temp.cr.skm
; Step 2: Condition & Split Skims into Multiple Files
;  Input Files: transit.temp.cr.skm
;  Output Files: %_iter_%_AM_WK_CR.SKM, %_iter_%_AM_WK_CR.STA,
; Step 3: AM Peak Drive Skims
;  Input Files: ZONEHWY.NET, MODE?_AM.TB, *.TB
;  Output Files: transit.temp.cr.skm
; Step 4: Condition & Split Skims into Multiple Files
;  Input Files: transit.temp.cr.skm
;  Output Files: %_iter_%_AM_DR_CR.SKM, %_iter_%_AM_DR_CR.STA,
; Step 5: Off Peak Walk Skims
;  Input Files: ZONEHWY.NET, MODE?_OP.TB, *.TB
;  Output Files: transit.temp.cr.skm
; Step 6: Condition & Split Skims into Multiple Files
;  Input Files: transit.temp.cr.skm
;  Output Files: %_iter_%_OP_WK_CR.SKM, %_iter_%_OP_WK_CR.STA,
; Step 7: Off Peak Drive Skims
;  Input Files: ZONEHWY.NET, MODE?_OP.TB, *.TB
;  Output Files: transit.temp.cr.skm
; Step 8: Condition & Split Skims into Multiple Files
;  Input Files: transit.temp.cr.skm
;  Output Files: %_iter_%_OP_DR_CR.SKM, %_iter_%_OP_DR_CR.STA
;
;---------------------------------------------------------------------------
; rm  4/7/08                      ;
; Added  table #19 (Total Transit time in min.) to output transit.temp.cr.skm file
; create total transit time skims named:                                
;         %_iter_%_@TIME_PERIOD@_@ACCESS_MODE@_CR.ttt                 
;
...
fileo supporto = supl_@time_period@_@access_mode@_cr.asc modes=11-16 oneway=t fixed=y
fileo nodeo    = supn_@time_period@_@access_mode@_cr.dbf
fileo linko    = trnl_@time_period@_@access_mode@_cr.dbf   ; Can be used to create transit shapefile
;
...
;@dp_token@distributeIntrastep processId='mwcog', ProcessList=%subnode%
 MATI[1]=transit.temp.cr.skm
 MATO[1]=%_iter_%_@TIME_PERIOD@_@ACCESS_MODE@_CR.SKM, MO = 1-16,
 ...
 MATO[2]=%_iter_%_@TIME_PERIOD@_@ACCESS_MODE@_CR.STA, MO = 17-18,
  NAME = I_STA, J_STA

 MATO[3]=%_iter_%_@TIME_PERIOD@_@ACCESS_MODE@_CR.ttt, MO = 100,                ;
  NAME = sumtrntm
