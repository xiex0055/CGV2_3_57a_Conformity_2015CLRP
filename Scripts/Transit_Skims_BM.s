...
;  Output Files: transit.temp.bm.skm
; Step 2: Condition & Split Skims into Multiple Files
;  Input Files: transit.temp.bm.skm
;  Output Files: %_iter_%_AM_WK_BM.SKM, %_iter_%_AM_WK_BM.STA,
; Step 3: AM Peak Drive Skims
;  Input Files: ZONEHWY.NET, MODE?_AM.TB, *.TB
;  Output Files: transit.temp.bm.skm
; Step 4: Condition & Split Skims into Multiple Files
;  Input Files: transit.temp.bm.skm
;  Output Files: %_iter_%_AM_DR_BM.SKM, %_iter_%_AM_DR_BM.STA,
; Step 5: AM Peak K/R Skims
;  Input Files: ZONEHWY.NET, MODE?_AM.TB, *.TB
;  Output Files: transit.temp.bm.skm
; Step 6: Condition & Split Skims into Multiple Files
;  Input Files: transit.temp.bm.skm
;  Output Files: %_iter_%_AM_KR_BM.SKM, %_iter_%_AM_KR_BM.STA,
; Step 7: Off Peak Walk Skims
;  Input Files: ZONEHWY.NET, MODE?_OP.TB, *.TB
;  Output Files: transit.temp.bm.skm
; Step 8: Condition & Split Skims into Multiple Files
;  Input Files: transit.temp.bm.skm
;  Output Files: %_iter_%_OP_WK_BM.SKM, %_iter_%_OP_WK_BM.STA,
; Step 9: Off Peak Drive Skims
;  Input Files: ZONEHWY.NET, MODE?_OP.TB, *.TB
;  Output Files: transit.temp.bm.skm
; Step 10: Condition & Split Skims into Multiple Files
;  Input Files: transit.temp.bm.skm
;  Output Files: %_iter_%_OP_DR_BM.SKM, %_iter_%_OP_DR_BM.STA
; Step 11: Off Peak K/R Skims
;  Input Files: ZONEHWY.NET, MODE?_OP.TB, *.TB
;  Output Files: transit.temp.bm.skm
; Step 12: Condition & Split Skims into Multiple Files
;  Input Files: transit.temp.bm.skm
;  Output Files: %_iter_%_OP_KR_BM.SKM, %_iter_%_OP_KR_BM.STA,
;
;---------------------------------------------------------------------------
; Added Mode 15 access links for KNR to Bus path, JainM 09.19.06
; @KR_MODEL@ READ FILE = bus@TIME_PERIOD@pnr.tb  ;---- Bus-PNR connectors (links)

; rm  4/7/08                      ;
; Added  table #19 (Total Transit time in min.) to output transit.temp.bm.skm file
; create total transit time skims named:                                  
;         %_iter_%_@TIME_PERIOD@_@ACCESS_MODE@_BM.ttt                  
...
;---------------------------------------------------------------------------
; Step 1, 3, 5 , 7, 9 & 11 Build Transit Path
;---------------------------------------------------------------------------

RUN PGM=TRNBUILD
 NETI = ZONEHWY.NET
 MATO = transit.temp.bm.skm
 maxnode = 60000
...
fileo supporto = supl_@time_period@_@access_mode@_bm.asc modes=11-16 oneway=t fixed=y
fileo nodeo    = supn_@time_period@_@access_mode@_bm.dbf
fileo linko    = trnl_@time_period@_@access_mode@_bm.dbf   ; Can be used to create transit shapefile
;
...
; MW[1] = TIME(1,6,8),
 MW[1] = TIME(1) * @_IBFTR@ +
         TIME(6) * @_IBFTR@ +
         TIME(8) * @_OBFTR@ ,                ;---- ivt-local bus    (0.01 min)
...
;@dp_token@distributeIntrastep processId='mwcog', ProcessList=%subnode%
 MATI[1]=transit.temp.bm.skm
 MATO[1]=%_iter_%_@TIME_PERIOD@_@ACCESS_MODE@_BM.SKM, MO = 1-16,
 ...
 MATO[2]=%_iter_%_@TIME_PERIOD@_@ACCESS_MODE@_BM.STA, MO = 17-18,
  NAME = I_STA, J_STA

 MATO[3]=%_iter_%_@TIME_PERIOD@_@ACCESS_MODE@_BM.ttt, MO = 100,                ;
  NAME = sumtrntm
...
JLOOP
 IF ((MW[1] + MW[2] + MW[6] = 0 ) || (MW[3]+MW[5]=0))
  MW[1] = 0
  MW[2] = 0
  MW[3] = 0
  MW[4] = 0
