...
See Supported Configurations
    ...
    2) ArcGIS 10.0             CUBE 6.1.0 SP1 (without ArcGIS runtime)          *NO*                BUG in ArcPy with ArcGIS 10.0 (Join doesn't work)
    3) ArcGIS  9.3             CUBE 6.1.0 SP1 (without ArcGIS runtime)          *NO*

    4) ArcGIS 10.1             CUBE 6.0.2     (without ArcGIS runtime)          YES                 Tested OK
    5) ArcGIS 10.0             CUBE 6.0.2     (without ArcGIS runtime)          *NO*                BUG in ArcPy with ArcGIS 10.0 (Join doesn't work)
...
__version__ = '2.3.52-2'      ## Version is a suffix to TPB Model Version
...
#
# include paths for ArcGIS 10.2 full installation (use forward slashes '/' in place of Windows default backward slashes '\', otherwise use two backward slahes '\\')
#
sys.path.append("C:\\Program Files (x86)\\ArcGIS\\Desktop10.2\\arcpy")
sys.path.append("C:\\Program Files (x86)\\ArcGIS\\Desktop10.2\\bin")

#
# include paths for ArcGIS 10.1 full installation (use forward slashes '/' in place of Windows default backward slashes '\', otherwise use two backward slahes '\\')
#
sys.path.append("C:\\Program Files (x86)\\ArcGIS\\Desktop10.1\\arcpy")
sys.path.append("C:\\Program Files (x86)\\ArcGIS\\Desktop10.1\\bin")

#
# include paths for ArcGIS 10.2 runtime installation (use forward slashes '/' in place of Windows default backward slashes '\', otherwise use two backward slahes '\\')
#
sys.path.append("C:\\Program Files (x86)\\ArcGIS\\Engine10.2\\arcpy")
sys.path.append("C:\\Program Files (x86)\\ArcGIS\\Engine10.2\\bin")

#
# include paths for ArcGIS 10.1 runtime installation (use forward slashes '/' in place of Windows default backward slashes '\', otherwise use two backward slahes '\\')
#
...
calc_type                                    = "AreaWalk"             # Options are 'PercentWalk' or 'AreaWalk'
...
my_TAZ_area_name                             = "TAZ_Area"                   # Used if exists otherwise created,   DOUBLE in Square Feet   (Do not set to 'Shape_Area')
User_Input_TAZLandArea_Shapefile             = os.path.abspath("..\\..\\..\\TPBTAZ3722_TPBMod\\TAZLand3722.shp")
my_join_uniq_id                              = 1                            # If Input TAZ shapefile has 'Shape_Area' already in it, make this '1', else '0'
...
# NOTE: Currently there is an issue with ArcGIS 10.1 which reports 'workspace or data source is read only' after creating a file geo-database on a network drive.
...
my_join_Fieldlist                            = [ "Shape_Area" ]                 # This is automatic geometry field created for every shapefile in geodatabase
...
def MWCOG_BufferAndExport(point_shape, temp_prefix_short, temp_prefix_long, startfresh, export):
    ...
    myendtime     = None
    myelapsedtime = None
    MWCOG_PrintWriter( "\n\tMWCOG_BufferAndExport (Started: " + str(mystarttime).split(".")[0] + ")" )
    MWCOG_PrintWriter( "\tShapefile  : " + str( ('..' + point_shape[(len(point_shape)-my_rightstringlength):]) if len(point_shape) > my_rightstringlength else point_shape) )
    ...
    try:
        ...
        if (startfresh == 1):
                ...
                if (my_fieldexists): pass
                        ...
                        arcpy.AddField_management(temp_TAZ, my_TAZ_area_name, "DOUBLE", 14, "", "", my_TAZ_area_name, "NULLABLE", "NON_REQUIRED")
        ...
        arcpy.Buffer_analysis(point_shape, short_Temp1, dist_short, my_buffer_sideType, my_buffer_endType, my_buffer_dissolveType, my_buffer_dissolveField)
        arcpy.Compact_management( my_GDB )

        arcpy.Buffer_analysis(point_shape,  long_Temp1,  dist_long, my_buffer_sideType, my_buffer_endType, my_buffer_dissolveType, my_buffer_dissolveField)
        arcpy.Compact_management( my_GDB )
        ...
        arcpy.AddField_management(short_Temp1, my_temp_buffer_dissolve_field_name, "SHORT", 1, "", "", my_temp_buffer_dissolve_field_name, "NULLABLE", "NON_REQUIRED")
        arcpy.CalculateField_management(short_Temp1, my_temp_buffer_dissolve_field_name, "0", "PYTHON", "")

        arcpy.AddField_management( long_Temp1, my_temp_buffer_dissolve_field_name, "SHORT", 1, "", "", my_temp_buffer_dissolve_field_name, "NULLABLE", "NON_REQUIRED")
        ...
        # join
        arcpy.JoinField_management(temp_TAZ, my_TAZ_name, short_Temp4, my_TAZ_name, my_join_Fieldlist) # Help: http://help.arcgis.com/en/arcgisdesktop/10.0/help/index.html#//001700000065000000

        # construct shape_area name
        if   my_join_uniq_id == 0:                                                 # Quirky, but this is how ArcGIS appends fields
            my_shape_area_field = "Shape_Area"                                     # Quirky, but this is how ArcGIS appends fields
        elif my_join_uniq_id == 1:                                                 # Quirky, but this is how ArcGIS appends fields
            my_shape_area_field = "Shape_Area_1"                                   # Quirky, but this is how ArcGIS appends fields
        elif my_join_uniq_id == 2:                                                 # Quirky, but this is how ArcGIS appends fields
            my_shape_area_field = "Shape_Area_12"                                  # Quirky, but this is how ArcGIS appends fields
        elif my_join_uniq_id == 3:                                                 # Quirky, but this is how ArcGIS appends fields
            my_shape_area_field = "Shape_Area_12_13"                               # Quirky, but this is how ArcGIS appends fields
        elif my_join_uniq_id == 4:                                                 # Quirky, but this is how ArcGIS appends fields
            my_shape_area_field = "Shape_Area_12_13_14"                            # Quirky, but this is how ArcGIS appends fields
        elif my_join_uniq_id == 5:                                                 # Quirky, but this is how ArcGIS appends fields
            my_shape_area_field = "Shape_Area_12_13_14_15"                         # Quirky, but this is how ArcGIS appends fields
        elif my_join_uniq_id == 6:                                                 # Quirky, but this is how ArcGIS appends fields
            my_shape_area_field = "Shape_Area_12_13_14_15_16"                      # Quirky, but this is how ArcGIS appends fields
        ...
        if   (calc_type == "PercentWalk") :
                arcpy.CalculateField_management(temp_TAZ, temp_prefix_short, my_calcfield_expression, "PYTHON", my_pctwlk_calcfield_codeblock)
        elif (calc_type == "AreaWalk") :
                arcpy.CalculateField_management(temp_TAZ, temp_prefix_short, my_calcfield_expression, "PYTHON", my_arawlk_calcfield_codeblock)
        else:
        ...
        # join
        arcpy.JoinField_management(temp_TAZ, my_TAZ_name, long_Temp4, my_TAZ_name, my_join_Fieldlist) # Help: http://help.arcgis.com/en/arcgisdesktop/10.0/help/index.html#//001700000065000000
        ...
        # select based on calculation type
        if   (calc_type == "PercentWalk") :
                arcpy.CalculateField_management(temp_TAZ, temp_prefix_long, my_calcfield_expression, "PYTHON", my_pctwlk_calcfield_codeblock)
        elif (calc_type == "AreaWalk") :
                arcpy.CalculateField_management(temp_TAZ, temp_prefix_long, my_calcfield_expression, "PYTHON", my_arawlk_calcfield_codeblock)
        else:
                arcpy.AddError("ERROR: Un-recognized calc_type specified!")
        ...
        if (export==1):
                ...
                arcpy.ExportXYv_stats(temp_TAZ, my_out_fields, "COMMA", User_Output_Walkshed_CSV_File, "ADD_FIELD_NAMES")
                ...
                for row in rows:
                     ...
                     for field in fieldList:
                          if field.name in FieldsToSkip: i += 1
                                ...
                                # write fields
                                if   field.name == my_TAZ_name.upper()      : f.write('%6d'      %                            row.getValue(field.name) )
                                elif field.name == my_TAZ_area_name.upper() : f.write('%10.4f'   % round( (row.getValue(field.name) / (5280*5280)), 4) )
                                elif i < len(fieldList)                     : f.write('%10.4f'   % round(                 row.getValue(field.name), 4) )
                     ...
                     my_num_taz                                                                  += 1
                     my_total_taz_area                                                           += round( (row.getValue(my_TAZ_area_name.upper()) / (5280*5280)), 4)
                ...
                if( calc_type == "AreaWalk" ):
                        MWCOG_PrintWriter( "\nSUMMARY REPORT:"                                                                                                                                                                                                )
                        MWCOG_PrintWriter( "\n\tNumber of TAZ Records in Output                                  : " + str('{:,}'.format(my_num_taz))                                                                                                         )
                        MWCOG_PrintWriter( "\tTotal TAZ LAND Area                                              : " + str('{:9.4f} sq. mi.'.format(my_total_taz_area))                                                                                         )
                        if( len(my_taz_list_zero_land_area) == 0 ) :                                                                                                                                                                                          
                                MWCOG_PrintWriter( "\tTAZs with Zero LAND Area                                         : NONE"                                                                                                                                )
                        else:                                                                                                                                                                                                                                 
                                MWCOG_PrintWriter( "\tTAZs with Zero LAND Area                                         : " + "(Count=" + str(len(my_taz_list_zero_land_area)) + ") " + ','.join(sorted(my_taz_list_zero_land_area, key=int))                  )
                        MWCOG_PrintWriter( "\n\tTotal TAZ Long-Walk Area                                         : " + str('{:9.4f} sq. mi. ({:6.2%} of TAZ Land Area)'.format(my_total_taz_walk_area, my_total_taz_walk_area/my_total_taz_area))             )
                        MWCOG_PrintWriter( "\tTotal TAZ Long-Walk (Peak Period) Area                           : " + str('{:9.4f} sq. mi. ({:6.2%} of TAZ Land Area)'.format(my_total_taz_walk_pk_area, my_total_taz_walk_pk_area/my_total_taz_area))         )
                        MWCOG_PrintWriter( "\tTotal TAZ Long-Walk (Off-Peak Period) Area                       : " + str('{:9.4f} sq. mi. ({:6.2%} of TAZ Land Area)'.format(my_total_taz_walk_op_area, my_total_taz_walk_op_area/my_total_taz_area))         )
                        if( len(my_taz_list_zero_walk_area) == 0 ) :                                                                                                                                                                                          
                                MWCOG_PrintWriter( "\tTAZs with Zero WALK Area                                         : NONE"                                                                                                                                )
                        else:                                                                                                                                                                                                                                 
                                MWCOG_PrintWriter( "\tTAZs with Zero WALK Area                                         : " + "(Count=" + str(len(my_taz_list_zero_walk_area)) + ") " + ','.join(sorted(my_taz_list_zero_walk_area, key=int))                  )
                        MWCOG_PrintWriter( "\n\tTotal MTLRTSHR Area                                              : " + str('{:9.4f} sq. mi. ({:6.2%} of TAZ Land Area)'.format(my_total_mtlrt_shr_area, my_total_mtlrt_shr_area/my_total_taz_area))           )
                        MWCOG_PrintWriter( "\tTotal MTLRTLNG Area                                              : " + str('{:9.4f} sq. mi. ({:6.2%} of TAZ Land Area)'.format(my_total_mtlrt_lng_area, my_total_mtlrt_lng_area/my_total_taz_area))             )
                        MWCOG_PrintWriter( "\tTotal ALLPKSHR Area                                              : " + str('{:9.4f} sq. mi. ({:6.2%} of TAZ Land Area)'.format(my_total_allpk_shr_area, my_total_allpk_shr_area/my_total_taz_area))             )
                        MWCOG_PrintWriter( "\tTotal ALLPKLNG Area                                              : " + str('{:9.4f} sq. mi. ({:6.2%} of TAZ Land Area)'.format(my_total_allpk_lng_area, my_total_allpk_lng_area/my_total_taz_area))             )
                        MWCOG_PrintWriter( "\tTotal ALLOPSHR Area                                              : " + str('{:9.4f} sq. mi. ({:6.2%} of TAZ Land Area)'.format(my_total_allop_shr_area, my_total_allop_shr_area/my_total_taz_area))             )
                        MWCOG_PrintWriter( "\tTotal ALLOPLNG Area                                              : " + str('{:9.4f} sq. mi. ({:6.2%} of TAZ Land Area)'.format(my_total_allop_shr_area, my_total_allop_shr_area/my_total_taz_area))             )
                        if( len(my_taz_shr_list_pk_less_than_op) == 0 ) :                                                                                                                                                                                     
                                MWCOG_PrintWriter( "\n\tTAZs with Short-Walk Less in Peak Period than in Off-Peak Period : NONE" )                                                                                                                            
                        else:                                                                                                                                                                                                                                 
                                MWCOG_PrintWriter( "\n\tTAZs with Short-Walk Less in Peak Period than in Off-Peak Period : " + "(Count=" + str(len(my_taz_shr_list_pk_less_than_op)) + ") " + ','.join(sorted(my_taz_shr_list_pk_less_than_op, key=int))      )
                        if( len(my_taz_lng_list_pk_less_than_op) == 0 ) :                                                                                                                                                                                
                                ...
                                MWCOG_PrintWriter( "\tTAZs with  Long-Walk Less in Peak Period than in Off-Peak Period : " + "(Count=" + str(len(my_taz_lng_list_pk_less_than_op)) + ") " + ','.join(sorted(my_taz_lng_list_pk_less_than_op, key=int)) + "\n" )
    except:
        #
    ...
    MWCOG_PrintWriter( "\tMWCOG_BufferAndExport (Finished: " + str(myendtime).split(".")[0] + ", Elapsed: " + str(myelapsedtime).split(".")[0] + ")" )
    return 0
...
if __name__ == '__main__':
    ...
    MWCOG_PrintWriter ("BEGIN ArcPy Walkshed Process (Started: " + str(mystarttime).split(".")[0] + ")")
    ...
    if( MWCOG_BufferAndExport( User_Input_All_DY_MetroLRT_Stops_Shapefile, "MTLRTShr",  "MTLRTLng", 1, 0) == 1 ): sys.exit(1)
    if( MWCOG_BufferAndExport(      User_Input_All_PK_All_Stops_Shapefile, "ALLPKShr",  "ALLPKLng", 0, 0) == 1 ): sys.exit(1)
    if( MWCOG_BufferAndExport(      User_Input_All_OP_All_Stops_Shapefile, "ALLOPShr",  "ALLOPLng", 0, 1) == 1 ): sys.exit(1)

    # if( MWCOG_BufferAndExport( User_Input_All_DY_MetroLRT_Stops_Shapefile, "MTLRTShr",  "MTLRTLng", 1, 1) == 1 ): sys.exit(1)    ## debug individual
    # if( MWCOG_BufferAndExport(      User_Input_All_PK_All_Stops_Shapefile, "ALLPKShr",  "ALLPKLng", 1, 1) == 1 ): sys.exit(1)    ## debug individual
    # if( MWCOG_BufferAndExport(      User_Input_All_OP_All_Stops_Shapefile, "ALLOPShr",  "ALLOPLng", 1, 1) == 1 ): sys.exit(1)    ## debug individual

    # if( MWCOG_BufferAndExport( User_Input_All_DY_MetroLRT_Stops_Shapefile, "MTLRTShr",  "MTLRTLng", 1, 0) == 1 ): sys.exit(1)    ## fast run for debugging
    # if( MWCOG_BufferAndExport( User_Input_All_DY_MetroLRT_Stops_Shapefile, "ALLPKShr",  "ALLPKLng", 0, 0) == 1 ): sys.exit(1)    ## fast run for debugging
    # if( MWCOG_BufferAndExport( User_Input_All_DY_MetroLRT_Stops_Shapefile, "ALLOPShr",  "ALLOPLng", 0, 1) == 1 ): sys.exit(1)    ## fast run for debugging

    myendtime     = datetime.datetime.now()
    myelapsedtime = myendtime - mystarttime

    MWCOG_PrintWriter ("\nEND ArcPy Walkshed Process (Finished: " + str(myendtime).split(".")[0] + ", Elapsed: " + str(myelapsedtime).split(".")[0] + ")")
